...
| F | Optimizer Deep Dive | 6 optimizers on CNN (+ Nadam simulated — did not complete in session): SGD family vs adaptive family, Adagrad's dying LR, AdamW's decoupled weight decay, Nesterov look-ahead |
...
Tab F (**Optimizer Deep Dive**) extends Tab C with 6 optimizers run on the CNN architecture (SGD, SGD+Nesterov, Adagrad, RMSprop, Adam, AdamW), plus Nadam shown as a simulated reference (Nadam did not complete in the actual lab session). Covers the full SGD family (SGD → SGD+Nesterov) and adaptive family (Adagrad → RMSprop → Adam → AdamW → Nadam). It includes a best-val-accuracy bar chart, per-optimizer annotations connecting internal mechanics to observed curve behaviour, and a 7-card mechanism panel with update rules for all optimizers. Based on the voluntary extension experiment from the course notebook.
